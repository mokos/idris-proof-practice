...
-- はじめての証明
add_2_3_is_5 : II + III = V
add_2_3_is_5 = Refl -- Idris処理系が勝手に計算してくれて、自動で証明完了
...
addSucc (S k) b = 
                             --↓ (S k) + (S b) = S(S k + b) を得たい。これを変形していく。
                             --↓ (+)関数の規則(Sの右寄せ)が勝手に自動適用される
                             --↓ k + (S (S b)) = S(k + S b)
  rewrite addSucc k (S b) in --↓ addSucc規則(a=k)で置き換えをすると
  Refl                       --   S (k + S b) = S (k + S b) というRefl(反射律)が得られる
                             -- 得たい式を変形していくと、常に成り立つReflだったので証明完了
...
-- a=kで成り立つとして、a=(S k)でも成り立つことを示す（数学的帰納法）
add交換則 (S k) b =        --↓ (S k) + b = b + (S k) を得たい。これを変形していく。
                           --↓ (+)関数の規則(Sの右寄せ)が勝手に自動適用される
                           --↓ k + (S b) = b + (S k) 
  ...
  Refl                     -- 得たい式を変形していくと、常に成り立つReflだったので証明完了
