...
ドモルガンnotnandを仮定すると排中律 :
  ({a, b : Type} -> Not (Not a, Not b) -> Either a b) -> 排中律
ドモルガンnotnandを仮定すると排中律 h = h 真かつ偽は矛盾

-- {a : Type} と小文字を使うと謎のエラー
排中律を仮定すると二重否定除去 :
...
ドモルガンnandを仮定すると弱排中律 : ドモルガンnand -> 弱排中律
ドモルガンnandを仮定すると弱排中律 h = or交換 $ h 真かつ偽は矛盾
...
-- 命題同士が比較可能で、推移律と反対称律が成り立つ。
-- 排中律を仮定するとダメットの法則が成り立つので、古典論理に含まれる。
ダメットの法則を仮定すると弱排中律 :
